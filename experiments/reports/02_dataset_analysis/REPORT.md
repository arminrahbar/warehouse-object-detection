...
|---|---|---|
| experiments/scripts/00_build_dataset_inventory.py | parse_label_file; build_dataset_index | Validate YOLO rows and build one deterministic image record |
| experiments/scripts/02_summarize_dataset.py | load_and_validate_sources; build_summary_tables | Reconcile granular and aggregate counts, then characterize the corpus |
| experiments/scripts/02_dataset_sampling.py | proportional_targets; rare_class_targets; enforce_rare_class_targets; build_sampling_evidence | Build and compare exact-size candidates, enforce final coverage |
| experiments/scripts/02_overlap_analysis.py | compute_overlap_for_boxes; build_overlap_evidence | Compute canonical pairwise ground-truth overlap and selected views |
| experiments/scripts/03_nms_threshold_sweep.py | load_sample_index; load_overlap_profile | Consume the fixed workload and overlap membership in Experiment 03 |
...
`experiments/scripts/00_build_dataset_inventory.py`,
...
`experiments/scripts/00_build_dataset_inventory.py`,
`experiments/scripts/02_summarize_dataset.py`,
`experiments/scripts/02_dataset_sampling.py`, and
`experiments/scripts/02_overlap_analysis.py`. Together they validate the
