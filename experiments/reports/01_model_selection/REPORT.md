...
|---|---|
| `experiments/scripts/01_model_comparison.py` — `load_and_validate_index`, `run_inference_for_model`, `evaluate_model`, `verify_run_directory` | Validate the corpus and assets, run both checkpoints, compute metrics, and verify the quality evidence |
...
| `experiments/scripts/01_benchmark_inference.py` — `stratified_density_sample`, `benchmark_paired`, `paired_source_group_bootstrap` | Build the deterministic paired runtime sample and latency intervals |
| `experiments/scripts/01_select_checkpoint.py` — `validate_quality_run`, `validate_runtime_run`, `paired_quality_bootstrap`, `apply_selection_rule` | Re-verify upstream evidence, estimate quality uncertainty, and apply the locked decision rule |
...
- `experiments/scripts/01_model_comparison.py` for full-corpus quality;
- `experiments/scripts/01_benchmark_inference.py` for paired runtime;
- `experiments/scripts/01_select_checkpoint.py` for the locked decision.
